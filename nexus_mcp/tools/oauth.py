...
async def nexus_oauth_login(
    ...
    """Start the OAuth2 authorization-code flow: returns the URL the user opens in a browser.

    Step 1 of the OAuth flow - run this first, then pass the redirect 'code'
    to nexus_oauth_exchange. Uses PKCE S256 + random state. Bearer tokens then
    take precedence over NEXUS_API_KEY for user-context mutations.
...
async def nexus_oauth_exchange(
    ...
    """Complete the OAuth2 flow: exchange the auth code for tokens. Requires a pending login from nexus_oauth_login.
...
async def nexus_oauth_status() -> str:
    ...
    Safe to call anytime - never exposes the access token itself, only a
    prefix and expiry metadata.
...
async def nexus_oauth_refresh() -> str:
    """Force a refresh of the OAuth access token via the refresh grant. Call it when a request returns 401.

    A 4xx refresh failure means the user revoked the application (see
    https://users.nexusmods.com/oauth/authorized_applications) and the stored
    tokens are cleared - log in again with nexus_oauth_login.
...
async def nexus_oauth_logout() -> str:
    """Log out of OAuth: delete stored tokens and fall back to NEXUS_API_KEY authentication.
...
async def nexus_update_mod_direct_download(
    ...
    """Enable or disable direct (no-ads) downloads on your own mod [v2 - no v1 quota] [OAuth required].

    Requires OAuth login (nexus_oauth_login + nexus_oauth_exchange) - this
    user-context mutation is rejected under apikey-only auth even for the mod
    owner. Useful for mod authors automating release pipelines.
